...
// handler. Trimmed copy of armies/testsetup_test.go because the helper
// is package-private to each verbs subpackage.
...
// battlesHandler stages the resolver chain (servers, worlds, showWorld
// for my_kingdom, world map for region names) plus Phase 9-specific
// routes that tests merge in via `extra`.
func battlesHandler(t *testing.T, extra map[string]http.HandlerFunc) http.HandlerFunc {
	t.Helper()
...
// setupBattlesSession builds a session scoped at (acme, spring-2026)
// with kingdom kgd-1 resolved via showWorld.my_kingdom.
func setupBattlesSession(t *testing.T, extra map[string]http.HandlerFunc) (*shell.Session, *bytes.Buffer) {
	t.Helper()
	s, o, _ := newTestSession(t, battlesHandler(t, extra))
