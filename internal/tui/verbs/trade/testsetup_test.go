...
// handler. Trimmed copy of armies/battles testsetup helpers — every
// verbs subpackage carries its own copy because the helper stays
// package-private.
...
// tradeHandler stages the resolver chain (servers, worlds, showWorld
// for my_kingdom, world map for region names) plus Phase 11-specific
// routes that tests merge in via `extra`.
func tradeHandler(t *testing.T, extra map[string]http.HandlerFunc) http.HandlerFunc {
	t.Helper()
...
// setupTradeSession builds a session scoped at (acme, spring-2026)
// with kingdom kgd-1 resolved via showWorld.my_kingdom.
func setupTradeSession(t *testing.T, extra map[string]http.HandlerFunc) (*shell.Session, *bytes.Buffer) {
	t.Helper()
	s, o, _ := newTestSession(t, tradeHandler(t, extra))
